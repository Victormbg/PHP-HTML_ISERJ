...
function conecta(){
	...
	print "Conectado ao banco de dados com sucesso<br>";
	return $conexao;
}

function inserir($conexao,$query){
	if(mysqli_query($conexao, $query)) {
		print "Inserido com sucessso no banco de dados<br>";
	}
	else{
		print "Erro não foi possivel inserir". mysqli_errno($conexao);
	}
}
...
function alterar($conexao,$query){
	if(mysqli_query($conexao, $query)) {
		...
		print "Erro não foi possivel alterar". mysqli_errno($conexao);
	}
}
function excluir($conexao,$query){
	if(mysqli_query($conexao, $query)) {
		...
		print "Erro não foi possivel excluir ". mysqli_errno($conexao);	
	}
}
$con=conecta();
